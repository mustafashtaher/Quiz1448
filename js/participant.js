...
async function searchParticipant(event) {
  ...
  try {
    ...
    const nameLabel = Utils.$('#search-name');
    const nameDisplay = Utils.$('#search-name-display');
    if (nameLabel) nameLabel.textContent = participant.FullName || 'Participant';
    Utils.show(nameDisplay);
    ...
    Utils.hide(Utils.$('#profile')); Utils.hide(Utils.$('#actions')); Utils.hide(Utils.$('#search-name-display'));
...
function renderQuiz(root) {
  const q = participantState.quiz.questions[participantState.index];
  const n = participantState.quiz.questions.length;
  const saved = participantState.answers[q.QuestionID] || '';
...
function explanationMarkup(result) {
  const details = result.answerDetails || result.answers || result.questionResults || [];
  if (Array.isArray(details) && details.length) {
    const incorrect = details.filter(item => item.Correct === false || item.correct === false || item.isCorrect === false);
    const explanations = incorrect.map(item => {
      const question = participantState.quiz.questions.find(q => String(q.QuestionID) === String(item.QuestionID || item.questionId));
      const text = item.Explanation || item.explanation || (question && question.Explanation);
      return text ? `<div class="explanation-item"><strong>Explanation</strong><p>${Utils.escape(text)}</p></div>` : '';
    }).filter(Boolean).join('');
    if (explanations) return `<div class="explanations"><h3>Review incorrect answers</h3>${explanations}</div>`;
  }
  if (Array.isArray(result.explanations) && result.explanations.length) return `<div class="explanations"><h3>Review</h3>${result.explanations.map(text => `<p>${Utils.escape(text)}</p>`).join('')}</div>`;
  return '';
}
