...
function renderProfile(p) {
  const fields = [
    ['Gender', 'Gender'], ['Age', 'Age'], ['Jamaat', 'Jamaat'], ['Jamiaat', 'Jamiaat'],
    ['Category', 'Category'], ['Idara', 'Idara'], ['Hifz_Sanad', 'Hifz / Sanad'], ['Occupation', 'Occupation']
  ].filter(([key]) => String(p[key] ?? '').trim() !== '');
  const profile = Utils.$('#profile');
  profile.innerHTML = `<div class="profile-grid">${fields.map(([key, label]) => `<div><small>${label}</small><strong>${Utils.escape(p[key])}</strong></div>`).join('')}</div>`;
  if (!fields.length) profile.innerHTML = '<p class="empty">No additional profile details available.</p>';
  Utils.show(profile); Utils.show(Utils.$('#actions'));
}
...
async function searchParticipant(event) {
  ...
  try {
    ...
    currentParticipant = participant; participantState.answers = {}; renderProfile(participant); Utils.setMessage(message, '', 'info');
...
async function submitQuestion(event) {
  ...
  try {
    const file = Utils.$('#question-image')?.files?.[0] || null; let image = null;
    ...
    await API.submitAudienceQuestion({ muminId: currentParticipant.Mumin_ID, name: currentParticipant.FullName || '', group: currentParticipant.Group_Name, questionType: Utils.$('#question-type').value.toLowerCase(), question: Utils.$('#question-text').value.trim(), image });
...
document.addEventListener('DOMContentLoaded', () => {
  Utils.$('#question-name')?.closest('label')?.remove();
  Utils.$('#participant-form')?.addEventListener('submit', searchParticipant); Utils.$('#start-quiz')?.addEventListener('click', openQuiz);
  ...
  Utils.$('#cancel-question')?.addEventListener('click', () => Utils.hide(Utils.$('#question-section'))); Utils.$('#audience-form')?.addEventListener('submit', submitQuestion);
