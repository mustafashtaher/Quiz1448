const API = (() => {
  async function call(action, payload = {}, token = null) {
    ...
    try {
      // GAS route functions read fields directly from data, not data.payload.
      ...
      if (result && result.success === false) {
        throw new Error(result.message || result.error || 'The server rejected this request.');
      }
      return result;
    } catch (error) {
  ...
  return {
    ...
    getQuiz: (id, group, day) => {
      const muminId = String(id || '').trim();
      return call('getQuiz', {
        Mumin_ID: muminId, muminId, Group_Name: group || '', groupName: group || '', group: group || '', day: day || ''
      });
    },
    submitQuiz: (payload) => {
      const value = payload || {};
      return call('submitQuiz', {
        ...
        groupName: value.groupName || value.group || value.Group_Name || '',
        group: value.group || value.Group_Name || '',
        day: value.day || '', startedAt: value.startedAt || '', submittedAt: value.submittedAt || '',
    ...
    submitAudienceQuestion: (payload) => {
      ...
      return call('submitAudienceQuestion', {
        Mumin_ID: value.Mumin_ID || value.muminId || '', muminId: value.muminId || value.Mumin_ID || '',
        Name: value.Name || value.name || '', name: value.name || value.Name || '',
        Group_Name: value.Group_Name || value.group || '', groupName: value.groupName || value.group || value.Group_Name || '', group: value.group || value.Group_Name || '',
        QuestionType: value.QuestionType || value.questionType || 'text', questionType: value.questionType || value.QuestionType || 'text',
        Question: value.Question || value.question || '', question: value.question || value.Question || '',
        File_ID: value.File_ID || value.fileId || '', fileId: value.fileId || value.File_ID || '', image: value.image || null, day: value.day || ''
      });
    },
    ...
    dashboard: (filters, token) => {
      ...
      return call('getDashboard', {
        ...
        group: value.group || value.groupName || value.Group_Name || 'ALL',
        groupName: value.groupName || value.group || value.Group_Name || 'ALL',
        Group_Name: value.Group_Name || value.group || value.groupName || 'ALL'
      }, token);
    },
